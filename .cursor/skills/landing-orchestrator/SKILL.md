...
11. **执行阶段**（SEO 建议生成后，按所选路径执行）：
    - **路径 1**：设计大师主导 → 产出设计规格 → 前端专家实现（参考 SEO 建议）
    - **路径 2**：前端专家主导 → 依据内容与结构直接实现（参考 SEO 建议）
    - **路径 3**：用户提供参考 → 设计大师参考后产出规格 → 前端专家实现（参考 SEO 建议）

12. **评分阶段**（每次生成完成后**必须执行**）：
    - 调用评分 Agent（landing-scorer）对本次生成的页面进行评分
    - 将评分报告写入同目录下的 `rating.md`

13. **合格性检查**（评分完成后**必须执行**）：
    - 读取 `rating.md`，检查评分结果是否达到合格标准：
...
14. **迭代优化流程**（仅当评分不合格时执行）：
    ...
    - **分析未达标维度**，确定需要优化的 Agent：
      - **需求符合度不达标**（<20/25）：
        - 产品专家（orchestrator）检查需求理解是否正确
        - 如需求理解有误，更新 `demand.md` 并重新进入生成流程
      - **UI/UX 规范不达标**（<24/30）：
        - 前端专家根据优化建议修复 UI/UX 规范问题
      - **设计质量不达标**（<16/20）：
        - **路径 1、3**：设计大师优化设计规格，前端专家重新实现
        - **路径 2**：前端专家直接优化设计质量
      - **技术实现不达标**（<8/10）：
        - 前端专家修复技术实现问题
      - **SEO 质量不达标**（<12/15）：
        - **5.1 SEO 专家输出质量不达标**：SEO 专家优化 `seo-suggestions.md`
        - **5.2 前端专家实施质量不达标**：前端专家优化页面 SEO 实施
    - **协调相关 Agent**：
...
15. **项目复盘阶段**（合格性检查通过后**必须执行**，无论是否经过迭代优化）：
    - **作为项目经理角色**，主动组织全体参与 Agent 进行项目复盘
    ...
    - **重要性**：项目复盘是项目完成的必要步骤，用于总结经验、识别问题、提升能力
